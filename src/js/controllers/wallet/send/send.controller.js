      ...
      if (request.isNotEmpty()) {
        ...
        if (PaymentRequest.PAYMENT_REQUEST === request.type) {
          ...
          if (!form.address.$isValid && !vm.blockUx) {
            ...
            vm.resetForm();
            vm.error = gettextCatalog.getString('Could not recognize a valid SimpaWallet QR Code');
          }
          ...
          if (request.state === 'PENDING') {
            vm.setForm(request.address, request.amount, null, ENV.TRAVELFLEX_ASSET, null, true);
