...
if [ "$(uname)" == "Darwin" ]; then
 PackagePath='../net.bg.simpa.androidbuilds/DagWallet/osx64/dagcoin-wallet.app/Contents/Resources/app.nw/'
 Action=dmg
elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
  if [ "$1" == "testnet" ]; then
    PackagePath='../net.bg.simpa.androidbuilds/dagcoin-TN-wallet/linux64/'
    Action=linux64:testnet
  else
    PackagePath='../net.bg.simpa.androidbuilds/DagWallet/linux64/'
    Action=linux64:live
  fi
fi
